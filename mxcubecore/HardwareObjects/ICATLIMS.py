...
class ICATLIMS(AbstractLims):
    ...
    def _create_icat_rest_session(self, user_name: str, password: str) -> IcatSession:
        try:
    ...
    def login(
        ...
        self._create_icat_session(password, username)
        rest_session = self._create_icat_rest_session(
            user_name=username, password=password
        )
        self._icat_session_dict[username] = rest_session

        # Connected to metadata icatClient
        msg = "Connected succesfully to ICAT: "
        msg += f"fullName={rest_session.full_name}, url={self.url}"
        logger.debug(msg)
        ...
        return self.session_manager, rest_session.name, sessions
            ...
            metadata.update(
                {
                    ...
                    MX_OSCILLATION_OVERLAP_KEY: oscillation_sequence["offset"],
